...
ChatView::ChatView(const Chat& chat) : currentChat(chat) {}

void ChatView::displayChat() const {
...
void ChatView::handleInput(const std::string& input) {
    if (input == "b") {
...
std::string ChatView::getMessageInput() const {
...
void ChatView::prevPage() {
    if (currentPage > 0) {
        currentPage--;
    }
}

void ChatView::firstPage() {
    currentPage = 0;
}

void ChatView::lastPage() {
    int totalPages = (currentChat.getMessages().getMessages().size() + MESSAGES_PER_PAGE - 1) / MESSAGES_PER_PAGE;
