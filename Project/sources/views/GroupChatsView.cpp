#include "GroupChatsView.h"
#include <iostream>
#include <iomanip>

GroupChatsView::GroupChatsView(const ChatContainer& container)
    : chatContainer(container) {}

void GroupChatsView::displayChats() const {
    std::cout << "#### Chats Menu ###\n\n";
...
void GroupChatsView::handleInput(const std::string& input) {
    if (input == "\n") {
        nextPage();
    } else if (input == "\t") {
        prevPage();
...
void GroupChatsView::prevPage() {
    if (currentPage > 0) {
        currentPage--;
    }
}

void GroupChatsView::firstPage() {
    currentPage = 0;
}

void GroupChatsView::lastPage() {
    int totalPages = (chatContainer.getChats().size() + CHATS_PER_PAGE - 1) / CHATS_PER_PAGE;
    currentPage = totalPages - 1;
}

std::string GroupChatsView::getSelectedChatId() const {
    std::vector<Chat> chats = chatContainer.getChats();
    int start = currentPage * CHATS_PER_PAGE;
